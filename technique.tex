...
\label{subsubsec:pre-analysis} Multiple benchmarks have defined fields as mock objects via annotations, or defined in the default \texttt{<init>} or \texttt{@Before} methods (\textit{setUp()} or \textit{init()}) at the beginning of test classes. These field type mock objects or mock-containing containers are later used in multiple test cases, supporting the actual object to be tested. The field mocks are collected by the pre-analysis of two transformers ahead of the main analysis, under the assumption that fields are scarcely mutated in the test cases. (Table~\ref{tab:mutations} displays the preliminary analysis results of field mutation frequency inside test cases. Less than 0.3\% of fields are mutated in test cases.)  

The first transformer takes care of the annotated field mocks and field mocks defined in default \texttt{<init>} method. 

The detection of annotated field mock is fairly easy. \textsc{MockDetector} will retrieve all the fields in the test class via Soot's Scene, and get the visibility annotation tag for each field, if possible. It then add the fields with annotation type of {\tt Lorg/mockito/Mock;} or {\tt Lorg/easymock/Mock;} to \texttt{annotatedMocks}. Listing~\ref{lis:annotatedMock} illustrates a Mockito annotated field mock example taken from \texttt{DefaultToolchainManagerTest.java} class in maven-core. \textsc{MockDetector} will store the three fields with \texttt{@Mock} into a HashSet, which later to be used in the main analysis.

Listing~\ref{lis:fieldMock} depicts an example where the field mocks are initialized at their declarations prior to the @Before method. These initializations would be included in default \texttt{<init>} method in Soot's Jimple IR. The detection of field mocks defined by field initializations (with known mock-creation source methods) are done by applying the forward dataflow analysis strictly on all default \texttt{<init>} methods in the test classes prior to the main analysis, where the same logic looking for values that are mock objects or mock-containing containers in the main analysis are applied.

The second pre-analysis transformer takes care of the field mocks defined in @Before methods, subsequent to the pre-analysis performed on default \texttt{<init>} methods.

Listing~\ref{lis:fieldMock2} illustrates an example where the fields are defined as mocks via mock-creation source methods inside the @Before method. Similarly, the field mocks initialized inside the @Before methods are determined by applying the forward dataflow analysis strictly on all @Before methods before the main analysis, where the values  
