...
Table~\ref{tab:mocks} outlines the number of test-related (Test/Before/After) methods that contain local variables or fields that are mocks, mock-containing arrays, or mock-containing collections, reported by Soot Intra-procedural analysis. Across the 8 open-source benchmarks, test-related methods containing local/field mocks or mock-containing containers constituted for between 0.35\% to 51.8\% of the total number of test-related methods found in public concrete test classes, via driver class injections. This suggests the benchmarks selected represent a wide range of real-world examples, holding different philosophies regarding the creation and usage of mock objects in tests. Benchmarks like \textsc{vraptor-core} and \textsc{jsonschema2pojo-core} entails more than half of their test-related methods with mock objects (and mock-containing array), which both relies heavily on annotations and thus having most of the field mocks created via annotations and reused in multiple test cases in the same class. 

Table~\ref{tab:invokes} highlights the number of method invocations on mocks that have been caught by both implementations. The two implementations have the same number of intraprocedural mock invocations for 4 open-source benchmarks experimented. Out of the ones with discrepancies, the main source of missed mock calls come from some of the missed support for array or collection related functions in Soot.
