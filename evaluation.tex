...
We have evaluated \textsc{MockDetector} on 8 open-source benchmarks, along with a micro-benchmark that we developed to test our tool. Table~\ref{tab:runtimes} presents the LOC and runtime information for each benchmark. The 9 benchmarks include over 383 kLOC, with 184 kLOC in the test suites measured by SLOCCount. Our Doop analysis's runtime for intra-procedural analysis is about 7$\times$ larger than our Soot analysis's runtime, which is in large part explained by the call graph that Doop builds on. %Add reference for SLOCCount.
