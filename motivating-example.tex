...
\paragraph{Basic declarative analysis} Again referring to Listing~\ref{lis:mockMethodIllustrationIR}, this time we ask whether the invocation on line~\ref{line:lis3line9} satisfies predicate \texttt{isMockInvocation}. It does, because that line contains a virtual method invocation, and the receiver object \texttt{r2} for the invocation on that line satisfies \texttt{isMockVar}. In this case, \texttt{r2} directly satisfies \texttt{isMockVar} (which we will explain later, in Listing~\ref{lst:core}), because line~\ref{line:lis3line6} assigns \texttt{r2} the return value from mock source method \texttt{createMock}, and the call to \texttt{createMock} satisfies predicate \texttt{callsMockSource}, which requires that the call destination \texttt{createMock} be enumerated as a constant in our 1-ary relation \texttt{MockSourceMethod}, and that there be a call graph edge between the method invocation at line~\ref{line:lis3line6} and the mock source method.
